...
//create function for  for Excel report 
   function fnExcelReport() { 
     ...
     $('#test').attr('download', '<?php echo $type;?>_report_detailed.xls');   
 }
</script>	
...
<?php 
		...
		//	case "facility_types" : $heading = $projects[0]->facility_type; break;
			...
			case "district_divisions" : 
					if($this->input->post('district_id')){
						...
						$heading = "Projects in ".$projects[0]->division; 
						break;
		...
		$pending_bills=0; $road_length_target = 0; $road_length_completed=0;
	?>
...
<!--	<th>Facility Name</th>
	...
	<tr style="<?php echo $color; ?>" onclick="$('#select_project_form_<?php echo $project->project_id;?>').submit();">
		...
		<!--<td><?php echo $project->facility_name; ?></td>
                <td><?php echo $project->facility_type; ?></td> -->
		...
		<td>
			...
			?>
                     <?php 
                        foreach($ho_pendencies as $ho_pendency){
                            if($ho_pendency->project_id == $project->project_id){
                                echo $ho_pendency->pendency_details."<br />";
                            }
                        }
                     ?>
		</td>
		...
		<td><?php echo $project->division; ?></td>
                <td><?php echo $project->agency_name; ?></td>
		<td><?php echo $project->user_department; ?></td>
		<td class="text-right"><?php echo $project->designation; ?> - <?php echo $project->staff_name;?></td>
		<td class="text-right"><?php echo $project->road_length_target; ?></td>
		<td class="text-right"><?php echo $project->road_length_completed; ?></td>
	</tr>
	<?php
		...
		$road_length_target+=$project->road_length_target;
		$road_length_completed+=$project->road_length_completed;
	}
	?>
	</tbody>
	<tr>
		...
		<th colspan="13"></th>
		<th><?php echo $road_length_target;?> Kms</th>
		<th><?php echo $road_length_completed;?> Kms</th>
	</tr>
	</table>
	</div>
	</div>
...
<table class="table table-hover table-bordered tablesorter sr-only" id="table-2">
	<thead>
	<th>S.No</th>
	<th>Project ID</th>
	<th>Project Name</th>
