...
//create function for  for Excel report 
   function fnExcelReport() { 
     ...
     tab_text = tab_text + $('#table-2').html(); 
     tab_text = tab_text + '</table></body></html>'; 
     var data_type = 'data:application/vnd.ms-excel'; 
     $('#test').attr('href', data_type + ', ' + encodeURIComponent(tab_text)); 
     //downloaded excel sheet name is given here 
     $('#test').attr('download', '<?php echo $type;?>_summary_report.xls');   
 }
</script>	
...
<div class="row">
	...
	<thead>
		...
		<th>Exp. Last FY</th>
		<th>Balace TS amount as on 1st April</th>
		...
		<small><?php echo date("M", mktime(0, 0, 0, $this->input->post('month')+1,  0, 0)).", ".$this->input->post('year');?></small>
		<?php } else{ echo date("M, Y"); } ?></th>
		<th>Cum. Exp As On Date</th>
		<th>Pending Bills</th>
		<th>Total Target for the year</th>
		<th>Exp % over TS</th>
		<th>Balance AS amount as on Date</th>
		<th>Total Works</th>
		<th>Not Started</th>
		<th>In Progress</th>
		<th>Completed</th>
		<th>Length Target (Kms)</th>
		<th>Length Completed (Kms)</th>
		</tr>
	</thead>
	<tbody>
	...
	$targets_total=0;
	$pending_bills=0;
	$road_length_target=0;
	$road_length_completed=0;
	
	$admin_sanction_amount_d=0;
	$tech_sanction_amount_d=0;
	$agreement_amount_d=0;
	$total_projects_d=0;
	$not_started_d=0;
	$work_in_progress_d=0;
	$work_completed_d=0;
	$medical_d=0;
	$non_medical_d=0;
	$expenses_d=0;
	$expenses_prev_d=0;
	$expenses_prev_year_d=0;
	$expenses_current_d=0;
	$expenses_current_month_d=0;
	$targets_current_d=0;
	$targets_current_month_d=0;
	$targets_total_d=0;
	$pending_bills_d=0;
	$road_length_target_d=0;
	$road_length_completed_d=0;
	$district_id = 0;
	...
	foreach($summary as $row){ 
		...
		if(!in_array($row->district_id,$districts) && $i>1){
		 ?>
			<tr onclick="$('#select_form_district_<?php echo $i;?>').submit();">
				<th>
					<?php echo form_open("reports/summary/$type",array('id'=>'select_form_district_'.$i,'role'=>'form')); ?>
						<input type='hidden' value="0" form="select_form_district_<?php echo $i; ?>" name="<?php echo $id;?>" />
						<input type='hidden' class="sr-only" value="<?php if($this->input->post('cumilative_report')) echo "1"; else echo "0"; ?>"  form="select_form_district_<?php echo $i; ?>" name="cumilative_report" />
						<?php if($this->input->post('state')) { ?>
						<input type='hidden' class="sr-only" value="<?php echo $this->input->post('state'); ?>" form="select_form_district_<?php echo $i; ?>" name="state" />
						<?php } ?>
						<?php if($this->input->post('year')) { ?>
						<input type='hidden' class="sr-only" value="<?php echo $this->input->post('year'); ?>"  form="select_form_district_<?php echo $i; ?>" name="year" />
						<?php } ?>
						<?php if($this->input->post('month')) { ?>
						<input type='hidden' class="sr-only" value="<?php echo $this->input->post('month'); ?>"  form="select_form_district_<?php echo $i; ?>" name="month" />
						<?php } ?>
						<input  type="hidden" value="<?php echo $district_id; ?>" form="select_form_district_<?php echo $i; ?>" name="district_id" />
					</form>
				</th>
				<th><?php echo $summary[$i-2]->district_name;?></th>
				<th>District Total</th>
				<th class="text-right"><?php echo number_format($admin_sanction_amount_d/10000000,2);?></th>
				<th class="text-right"><?php echo number_format($tech_sanction_amount_d/10000000,2);?></th>
				<th class="text-right"><?php echo number_format($agreement_amount_d/10000000,2);?></th>
				<th class="text-right"><?php echo number_format($expenses_prev_d/10000000,2);?></th>
				<th class="text-right"><?php echo number_format($expenses_prev_year_d/10000000,2);?></th>
				<th class="text-right"><?php echo number_format(($tech_sanction_amount-$expenses_prev_d)/10000000,2);?></th>
				<th class="text-right"><?php echo number_format($expenses_current_d/10000000,2);?></th>
				<th class="text-right"><?php echo number_format($targets_current_d/10000000,2);?></th>
				<th class="text-right"><?php echo number_format(($expenses_current_d/$targets_current_d)*100,2);?>%</th>
				<th class="text-right"><?php echo number_format($expenses_current_month_d/10000000,2);?></th>
				<th class="text-right"><?php echo number_format($targets_current_month_d/10000000,2);?></th>
				<th class="text-right"><?php echo number_format($expenses_d/10000000,2);?></th>
				<th class="text-right"><?php echo number_format($pending_bills_d/10000000,2);?></th>
				<th class="text-right"><?php echo number_format($targets_total_d/10000000,2);?></th>
				<th class="text-right"><?php echo number_format(($expenses_d/$tech_sanction_amount_d)*100,2);?>%</th>
				<th class="text-right"><?php echo number_format(($admin_sanction_amount_d-$expenses_d)/10000000,2);?></th>
				<th class="text-right"><?php echo $total_projects_d;?></th>
				<th class="text-right"><?php echo $not_started_d;?></th>
				<th class="text-right"><?php echo $work_in_progress_d;?></th>
				<th class="text-right"><?php echo $work_completed_d;?></th>
				<th class="text-right"><?php echo $road_length_target_d;?></th>
				<th class="text-right"><?php echo $road_length_completed_d;?></th>
			</tr>
	...
	<tr onclick="$('#select_form_<?php echo $row->$id;?>').submit();">
		...
		<?php } ?>
		<?php if($this->input->post('work_type')) { ?>
		<input type='hidden' class="sr-only" value="<?php echo $this->input->post('work_type'); ?>" name="work_type" />
		<?php } ?>
		...
		<td class="text-right"><?php echo number_format($row->expenses_previous_year/10000000,2); ?></td>  <!-- //Mark -->
		...
		<td class="text-right"><?php echo number_format($row->expense_current_month/10000000,2); ?></td> <!--  //Mark2 -->
		...
		<td class="text-right"><?php echo $row->road_length_target; ?>
		<td class="text-right"><?php echo $row->road_length_completed; ?>
		 </form>
		</td>
	</tr>
	<?php
	...
	$road_length_target += $row->road_length_target;
	$road_length_completed += $row->road_length_completed;

	
	$admin_sanction_amount_d+=$row->admin_sanction_amount;
	$tech_sanction_amount_d+=$row->tech_sanction_amount;
	$agreement_amount_d+=$row->agreement_amount;
	$total_projects_d+=$row->total_projects;
	$not_started_d+=$row->not_started;
	$work_in_progress_d+=$row->work_in_progress;
	$work_completed_d+=$row->work_completed;
	$expenses_prev_d+=$row->expenses_last_year;
	$expenses_prev_year_d+=$row->expenses_previous_year;
	$expenses_current_d+=$row->expenses_current_year;
	$expenses_current_month_d+=$row->expense_current_month;
	$targets_current_d+=$row->targets_current_year;
	$targets_current_month_d+=$row->target_current_month;
	$expenses_d+=$row->expenses_last_year+$row->expenses_current_year;
	$targets_total_d+=$row->targets_total_year;
	$pending_bills_d+=$row->bills_pending;
	$road_length_target_d += $row->road_length_target;
	$road_length_completed_d += $row->road_length_completed;
	...
	<tr onclick="$('#select_form_all').submit();">
		...
		<th class="text-right"><?php echo $road_length_target;?></th>
		<th class="text-right"><?php echo $road_length_completed;?></th>
        ...
        <table class="table table-hover table-striped table-bordered tablesorter sr-only" id="table-2">
	<thead>
		<tr>
		<th data-priority="critical">S.No</th>
		<th data-priority="critical" data-name="name"><?php echo $name;?></th>
	...
	<tr onclick="$('#select_form_<?php echo $row->$id;?>').submit();">
		...
		<td class="text-right"><?php echo $row->work_in_progress; ?></td>
		<td class="text-right"><?php echo $row->work_completed; ?></td>
		<td class="text-right"><?php echo $row->road_length_target; ?></td>
		<td class="text-right"><?php echo $row->road_length_completed; ?></td>
	</tr>
	<?php
	$admin_sanction_amount+=$row->admin_sanction_amount;
	$tech_sanction_amount+=$row->tech_sanction_amount;
