...
class Masters extends CI_Controller {
	...
	function edit($type=""){
        ...
        function update_grant(){
            ...
            if ($this->form_validation->run() === FALSE)
            {
                $this->data['msg'] = "Please input Grant Name";
            }
            else if($this->input->post('update')){
                $this->data['msg'] = $this->masters_model->update_grant() ? "Update Successful" : "Update Failed try again";
            }
            else
            {
                $grant = $this->masters_model->get_grant();
                if($grant && !empty($grant)){
                    $this->data['grant'] = $grant;
                }else{
                    $this->data['msg'] = "Please select Grant";                    
                }
            }
            $this->data['title']=$title;
            $this->data['grant_sources']=$this->masters_model->get_data("grant_sources");
            $this->data['grant_names'] = $this->masters_model->get_grant();
            $this->data['grant_phases'] = $this->masters_model->get_grant_phases();
            $this->load->view('templates/header',$this->data);
            $this->load->view('templates/left_nav');
