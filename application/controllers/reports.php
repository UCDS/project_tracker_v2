...
class Reports extends CI_Controller {
	...
	public function summary($type=0){
		...
		if ($this->form_validation->run() === FALSE)
		{
			...
			if($type == "district_divisions") 
				$this->load->view('pages/report_district_divisions_summary',$this->data);
			else $this->load->view('pages/report_summary',$this->data);
		}
		else{

			if($this->input->post('month')){
                            $this->data['ho_pendencies']=$this->staff_model->get_ho_pendency();
			...
			else if($this->input->post("$id") || $this->input->post('district_id')){
                $this->data['ho_pendencies']=$this->staff_model->get_ho_pendency();
				$this->data['pendencies']=$this->staff_model->get_pendencies(0,$this->data['user_departments'],$this->data['divisions']);                                
				$this->data['projects']=$this->staff_model->get_projects($this->data['user_departments'],$this->data['divisions']);
				$this->load->view('pages/report_detailed',$this->data);
	...
	public function dashboard(){
		$access=0;
		foreach($this->data['functions'] as $f){
			if($f->user_function=="Reports - Dashboard" && $f->view==1){
				$access=1;
			}
		}
		if($access == 0) show_404();
		if($this->session->userdata('logged_in')){
			$this->data['title']="APPRED Dashboard";
			$this->data['dashboard']=$this->reports_model->dashboard();
			$this->data['finance_report']=$this->reports_model->finance_report();
			$this->load->view('templates/header',$this->data);			
			$this->load->view('pages/report_graph',$this->data);
			$this->load->view('templates/footer');
		}
		else show_404();			
	}
