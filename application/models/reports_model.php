...
class Reports_model extends CI_Model{
	...
	function get_summary_report($type,$user_departments=0,$states=0,$divisions=-1){
		...
		if($this->input->post('cumilative_report')){
			...
			$final_bill_condition = " AND (IF(final_bill=1,IF(final_bill_date>='$year_start' AND final_bill_date<='$year_end',1,0),1)) ";
		...
		SUM(road_length_target) road_length_target,
		SUM(road_length_completed) road_length_completed,
		SUM(expenses_last_year) expenses_last_year,
		SUM(expenses_previous_year) expenses_previous_year,
		SUM(expenses_current_year) expenses_current_year,
		SUM(targets_current_year) targets_current_year,
		SUM(expense_current_month) expense_current_month,
		SUM(target_current_month) target_current_month, 
		SUM(targets_total_year) targets_total_year,
		SUM(bills_pending) bills_pending,
		...
		->join("(SELECT project_id, 
			...
			SUM(CASE WHEN month(project_expenses.expense_date)='$month' AND YEAR(project_expenses.expense_date)='$year'  THEN expense_amount ELSE 0 END) expense_current_month
			 FROM project_expenses JOIN projects USING(project_id) WHERE 1 $final_bill_condition  GROUP BY project_id) table_expenses" ,'projects.project_id=table_expenses.project_id','left')
		->join("(SELECT project_id, 			
		...
		->where('current',1);
		switch($type){
			case "district_divisions" :
					$this->db->select('district_name, divisions.district_id,divisions.division,divisions.division_id');
					 $this->db->group_by('divisions.division_id');
					 $this->db->order_by('district_name');					
					 break;
			case "divisions" :
					$this->db->select('divisions.division,divisions.division_id');
					$this->db->group_by('divisions.division_id');
					$this->db->order_by('divisions.division');					
					break;
			case "districts" :
			...
			case "facility_types" :
					...
					$this->db->order_by('facility_type');					
					break;
			case "facilities" :
			...
			case "staff" :
					...
					staff.staff_id,staff.reporting_officer_id,divisions.division_id,divisions.division,staff_category,staff_role,staff.designation",false);
	...
	function finance_report(){
		$this->db->select('budget_id,grant_name,(budget/10000000) as budget, (released/10000000) as released')->from('budget')->join('grants','budget.grant_id = grants.grant_id');
		$query = $this->db->get();
		return $query->result();
	}
	...
	function dashboard(){
		...
		$this->db->select('
		SUM(CASE WHEN (status_type_id = 1 AND IF(agreement_extension_date!="0000-00-00", agreement_extension_date, agreement_completion_date)>=CURDATE()) THEN 1 ELSE 0 END) as not_started_within,
		SUM(CASE WHEN (status_type_id = 1 AND IF(agreement_extension_date!="0000-00-00", agreement_extension_date, agreement_completion_date)<CURDATE()) THEN 1 ELSE 0 END) as not_started_beyond,
		SUM(CASE WHEN (status_type_id = 2 AND IF(agreement_extension_date!="0000-00-00", agreement_extension_date, agreement_completion_date)>=CURDATE()) THEN 1 ELSE 0 END) as in_progress_within,
		SUM(CASE WHEN (status_type_id = 2 AND IF(agreement_extension_date!="0000-00-00", agreement_extension_date, agreement_completion_date)<CURDATE()) THEN 1 ELSE 0 END) as in_progress_beyond,
		SUM(CASE WHEN (status_type_id = 3 AND IF(agreement_extension_date!="0000-00-00", agreement_extension_date, agreement_completion_date)>=actual_completion_date) THEN 1 ELSE 0 END) as completed_within,
		SUM(CASE WHEN (status_type_id = 3 AND IF(agreement_extension_date!="0000-00-00", agreement_extension_date, agreement_completion_date)<actual_completion_date) THEN 1 ELSE 0 END) as completed_beyond,
		SUM(road_length_target) target_length,
		SUM(road_length_completed) completed_length,
		SUM(CASE WHEN agreement_date < "'.$year_start.'" THEN agreement_amount ELSE 0 END)/10000000 previous_agreement_amount,
		SUM(CASE WHEN agreement_date >= "'.$year_start.'" THEN agreement_amount ELSE 0 END)/10000000 current_agreement_amount,
		SUM(CASE WHEN expense_date<"'.$year_start.'" THEN expense_amount ELSE 0 END)/10000000 previous_expenses,
		SUM(CASE WHEN expense_date>="'.$year_start.'" THEN expense_amount ELSE 0 END)/10000000 current_expenses,
		SUM(bills_pending)/10000000 pending_bills,
		SUM(targets_remaining_year)/10000000 targets_remaining_year
		...
		->join("(SELECT project_id, 
		SUM(bill_amount) bills_pending
		FROM project_bills WHERE active=1  GROUP BY project_id) table_bills" ,'projects.project_id=table_bills.project_id','left',false)
 		->join('(SELECT project_id, 			
		SUM(CASE WHEN target_amount!=0 AND projection_month>="'.date("Y-m-01").'"  THEN target_amount ELSE 0 END) targets_remaining_year
			 FROM project_targets WHERE current=1  GROUP BY project_id) table_targets' ,"projects.project_id=table_targets.project_id","left",false)
